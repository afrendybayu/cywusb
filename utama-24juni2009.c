...
//*	
	for(;;) {
		nyala = bacaStatusRX();
		if (nyala & 0x01 || nyala & 0x02) { // FULL A || EOF A
			tlsEnaRX(0x03);			// Enable EOF and FULL interrupts for channel A
			if (nyala & 0x08) { // Valid A
				nyala = bacaDataRX_A();
				transmit(nyala);
			}
			//data = CYWM_ReadReg( REG_RX_VALID_A );
...
//*
	...
	y = bacaDataRX_A();
	sprintf(x, "isi Data     : %s%c", y, CR); transmitString(x);
...
ISR(INT0_vect) {      // PORTD.2 pin no 4.  // ~ 38,5 us     = 292 clock  ?? 163 us
	cli();
	sprintf(x, "INT0 MLP%c", CR); transmitString(x);
	blink();
...
//*/	
    ...
    sei();
   	...
   	//sprintf(x, "MLP%c", CR); transmitString(x);

}


ISR(INT1_vect) {      // PORTD.2 pin no 4.  // ~ 38,5 us     = 292 clock  ?? 163 us
	char y;
	cli();
...
/*    
    printf("Reg ID         : %x%c%c", bacaRegID(), CR, NL);
    printf("Clock Manual   : %x%c%c", tlsSingleClockManual(0x41), CR, NL);
    printf("Clock Enable   : %x%c%c", tlsSingleClockEna(0x41), CR, NL);
    printf("Enable SerDes  : %x%c%c", tlsSingleSERDES(0x08 | 0x03), CR, NL);
    printf("Valid TX       : %x%c%c", tlsSingleValidTX(0xFF), CR, NL); 
    printf("VCO Cal        : %x%c%c", tlsSingleVcoCal(0x80), CR, NL);      // 0x80, 0xC0
    printf("Analog Control : %x%c%c", tlsSingleAnalogCtrl(0x04), CR, NL);
    printf("Kanal          : %x%c%c", (tlsSingleKanal(0x42)), CR, NL);
    printf("PA Bias        : %x%c%c", tlsSinglePABias(0x07), CR, NL); 
    //printf("Ena TX Int     : %x%c%c", tlsEnaTX(0x0F), CR, NL);
    printf("Ena RX Int     : %x%c%c", tlsEnaRX(0xFF), CR, NL);          // baca data A dan B
    printf("Kontrol RX TX  : %x%c%c", tlsSingleKontrol(0xF0), CR, NL);      
//*/    
} 
